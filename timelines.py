...
class TimelinesManager:
    def __init__(self, client: discord.Client, guilds: dict[int, Guild], logging_manager: LoggingManager):
        self.client = client
        self.guilds = guilds
    ...
    async def update_rolling_timelines(self, guild_id: int, user_id: int, event: ReactionEvent) -> None:
        ...
        if event.is_add:
            rolling = self.rolling_add # uses mutability of dict. rolling is just a pointer
        else:
        ...
        while rolling[(guild_id, user_id)] and rolling[(guild_id, user_id)][0] < current_time - self.guilds[guild_id].limits.interval_long:
            rolling[(guild_id, user_id)].popleft()

        # long interval check
        if len(rolling[(guild_id, user_id)]) > self.guilds[guild_id].limits.threshold_long:
            await self.handle_spam(guild_id, user_id)
            return

        # short interval check
        short_rolling = [1 for t in rolling[(guild_id, user_id)] if t >= current_time - self.guilds[guild_id].limits.interval_short]
